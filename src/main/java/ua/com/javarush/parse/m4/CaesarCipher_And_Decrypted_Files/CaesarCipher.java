...
public class CaesarCipher {
    ...
    void caesarCipher_ENGLISH(String text, int key){
        try {
            for (int i = 0; i < text.length(); i++) {
                ...
                if (IGNORED_CHARS.contains(currentChar)) {
                    ...
                    int newIndex = (index + key) % Symbols_And_Letters.ALPHABET_EN_UPPER.size();
                    ...
                    int newIndex = (index + key) % ALPHABET_EN_SMALLER.size();
    ...
    void caesarCipher_UKRAIN(String text, int key){
        try {
            for (int i = 0; i < text.length(); i++) {
                char currentChar = text.charAt(i);
                if (IGNORED_CHARS.contains(currentChar)) {
                    ...
                    int newIndex = (index + key) % ALPHABET_UA_UPPER.size();
                    ...
                    int newIndex = (index + key) % ALPHABET_UA_SMALLER.size();
